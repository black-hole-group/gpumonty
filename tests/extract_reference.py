...
def make ():
    ...
    try:
        print(run(["make", "-C", grmonty_path]).stdout)
...
def run_and_extract():
    ...
    for dump in dumps:
        ...
        for size, rounds in sizes:
            print("[EXTRACTOR]----------------------- Running " + dump + " N=" + str(size))
            infos = []
            spects = []
            for round in range(rounds):
                print("[EXTRACTOR]----------------------- Rounds: " + str_range(round, rounds))
                try:
                    process = run([grmonty_path + exec_name, str(size), dump, str(M_unit)], timeout=exec_timeout)
