...
void init_harm_data(char *fname)
{
    ...
    double rundt;
    int defcoord, MBH, QBH;
    int is, ie, js, je, ks, ke, whichdump, whichdumpversion, numcolumns;
	...
	fscanf(fp, "%d ", &ks);
	fscanf(fp, "%d ", &ke);
	fscanf(fp, "%d ", &whichdump);
	fscanf(fp, "%d ", &whichdumpversion);
	fscanf(fp, "%d ", &numcolumns);

	/************************************
    fscanf(fp, "%lf ", &rbr); 400.
    fscanf(fp, "%lf ", &npow2); 4.
    fscanf(fp, "%lf ", &cpow2); 1.
