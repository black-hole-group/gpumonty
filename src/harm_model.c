		    ...
		    double Ucon[NDIM], double Bcon[NDIM])
{
	...
	*Ne = p[NPRIM_INDEX3D(KRHO, i, j, k)] * NE_UNIT;
	*Thetae = p[NPRIM_INDEX3D(UU, i, j, k)] / (*Ne) * NE_UNIT * Thetae_unit;

	Bp[1] = p[NPRIM_INDEX3D(B1, i, j, k)];
	Bp[2] = p[NPRIM_INDEX3D(B2, i, j, k)];
	Bp[3] = p[NPRIM_INDEX3D(B3, i, j, k)];

	Vcon[1] = p[NPRIM_INDEX3D(U1, i, j, k)];
	Vcon[2] = p[NPRIM_INDEX3D(U2, i, j, k)];
	Vcon[3] = p[NPRIM_INDEX3D(U3, i, j, k)];
		      ...
		      double Bcov[NDIM])
{
	...
	double interp_scalar(double *var, int mmenemonics, int i, int j, int k, double del[8]);
