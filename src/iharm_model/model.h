...
#define MODEL_H
    ...
    */
    ...
    /**
     * @brief Wrapper function to select between standard coordinate mapping and simcoords algorithm.
     * 
     * @param i Integer grid index in the radial direction.
     * @param j Integer grid index in the polar direction.
     * @param k Integer grid index in the azimuthal direction.
     * @param X Output array to store the resulting coordinates \f$ X^\mu \f$.
     * @param use_simcoords Flag to determine whether to use the simcoords algorithm (1) or the standard coordinate mapping (0).
     * 
     */
    static __inline__ __host__ __device__ __attribute__((always_inline)) 
    void coord_wrapper(const int i, const int j, const int k, double *X, int use_simcoords, double *d_ks_r, double *d_ks_th)
    {
        if (use_simcoords) {
            simcoordijk_to_eks(i, j, k, X, d_ks_r, d_ks_th);
        } else {
            coord(i, j, k, X);
        }
    };
