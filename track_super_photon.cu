...
__device__ void track_super_photon_device(struct of_photon *ph);

__global__ void track_super_photon(struct of_photon *photons)
{
	size_t me = threadIdx.x + blockDim.x * blockIdx.x;

	if(me > 1) return;

	track_super_photon_device(&photons[me]);
